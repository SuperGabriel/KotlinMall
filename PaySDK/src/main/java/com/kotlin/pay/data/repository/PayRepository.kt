...
class PayRepository @Inject constructor() {
    ...
    fun getPaySign(orderId: Int, totalPrice: Long): Observable<BaseResp<String>> {
        return RetrofitFactory.create(PayApi::class.java).getPaySign(GetPaySignReq(orderId, totalPrice))
    }
    ...
    fun payOrder(orderId: Int): Observable<BaseResp<String>> {
        return RetrofitFactory.create(PayApi::class.java).payOrder(PayOrderReq(orderId))
    }
